...
def grade_evidence_node(state: AgentState) -> dict:
    ...
    context = "\n\n --- \n\n".join(item.document.page_content for item in retrieved)

    raw_result = grader.invoke(
        GRADE_EVIDENCE_PROMPT.format(
...
def rewrite_query_node(state: AgentState) -> dict:
    ...
    context_summary = "\n\n".join(
        item.document.page_content[:500] for item in retrieved
    )

    raw_result = query_rewriter.invoke(
        REWRITE_QUERY_PROMPT.format(
            question=state["question"],
            search_query=state.get("search_query", state["question"]),
